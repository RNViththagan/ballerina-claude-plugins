 ...
 *
 * Tools:
 *   search_libraries  — find libraries by keywords (libraryManager/getLibrariesBySearch)
 *   get_library       — get full API details for libraries (libraryManager/getFilteredLibraries)
 */
...
if (!process.env.LOCAL_BAL_LS_JAR) {
  ...
  const LS_BUILD = "REDACTED_LOCAL_PATH";
  const jarDir = `${LS_BUILD}/language-server/modules/language-server-extension/build/libs`;
  try {
    const jars = readdirSync(jarDir).filter(f =>
      f.startsWith("language-server-extension") && f.endsWith(".jar") &&
  ...
  async ({ keywords }) => {
    try {
      const result = await ls.request(
        "libraryManager/getLibrariesBySearch",
        { keywords }
      );
...
server.tool(
  ...
  async ({ libNames }) => {
    try {
      const result = await ls.request(
        "libraryManager/getFilteredLibraries",
        { libNames }
      );
